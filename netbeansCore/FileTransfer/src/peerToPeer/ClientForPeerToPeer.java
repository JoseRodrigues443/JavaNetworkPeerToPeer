 ...
 */
public class ClientForPeerToPeer implements ClientInterface {

    private int idClient;
    private Server ownServerPeerToPeer;
    ...
    private TcpFileList tcpFileList;
     ...
     * @param server
     */
    public ClientForPeerToPeer(Address clientAddress, boolean generateTcp, Server server) {
        this.ownServerPeerToPeer = server;
        ...
        this.tcpFileList = new TcpFileList(getOwnServerPeerToPeer().getIdServer());
        run();
    }
    ...
    public TcpFileList getTcpFileList() {
        return tcpFileList;
    }

    public void setTcpFileList(TcpFileList tcpFileList) {
        this.tcpFileList = tcpFileList;
    }

    public UDPClientStringManager getListen() {
        return listen;
    }

    public void setListen(UDPClientStringManager listen) {
        this.listen = listen;
    }

    public Server getOwnServerPeerToPeer() {
        return ownServerPeerToPeer;
    }

    public void setOwnServerPeerToPeer(Server ownServerPeerToPeer) {
        this.ownServerPeerToPeer = ownServerPeerToPeer;
    }
...
//            toAdd = cicle.replaceAll("«[0-9]*»", "");
//            //toAdd.trim();
//            toReturn.addValidated(toAdd);
//        }
//        return toReturn;
//    }
//
...
//    }
    ...
    /**
     * RECEVES a string like:
     *
     * 192.168.1.102;192.168.1.255;0;Destroi Salsicha.mp4«410224»; Puppy
     * Photo.jpg«171402»;doggy Video.mp4«1073391»;
    ...
    public boolean updateFileList(String announcementFromUDP) {
        ...
        String toAdd = "";
        Address a = new Address(IpProtocol.IPV4, fileNames[0], this.clientAddress.getMaskAddress(),
                this.clientAddress.getBroadcastAddress(), this.clientAddress.getUdpPort(),
                this.clientAddress.getTcpPort());
        /**/
        if (fileNames.length < 3) {
            /*UDP sented without files*/
            return false;
        }
        int tcpPort = Integer.parseInt(fileNames[2]);
        ArrayList<String> tmpArrayList = new ArrayList<>();
        for (int i = 3; i < fileNames.length; i++) {
            toAdd = fileNames[i];
            tmpArrayList.add(toAdd);

        }
        if (tcpFileList.cointainsTcp(tcpPort)) {
            tcpFileList.updateFileList(tcpPort, tmpArrayList);
        } else {
            tcpFileList.addValidated(new TcpFiles(tcpPort, a, tmpArrayList));
        }
     ...
     * @return
     */
    public FileList filesDownloaded() {
     ...
     * @return
     */
    public boolean sendDownloadRequestTcp(int tcpPortToSend, String nodeToSendAddress, ArrayList<String> fileList) {
        ...
        TextConnectionTCP cPClientSender = new TextConnectionTCP(tcpPortToSend);
    ...
    public boolean downloadFileAndUnzip(String fileName, Address address) {
        ...
        File file = cP.receiveFile(fileF.getAbsolutePath(), address);
